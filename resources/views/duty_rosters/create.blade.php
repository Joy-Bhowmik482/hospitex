...
<div class="max-w-6xl mx-auto py-8">
    <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-3xl font-bold text-slate-900">Create Duty Roster</h1>
            <p class="text-slate-600 mt-2">Assign doctors, staff, shifts, wards, rooms, and tasks for a professional roster.</p>
        </div>
    ...
    <div class="rounded-3xl border border-slate-200 bg-white shadow-sm p-8">
        <form action="{{ route('duty-rosters.store') }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <div class="space-y-3">
                    ...
                    <select id="day_of_week" name="day_of_week" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500 focus:ring-blue-500">
                        ...
                        @foreach([1 => 'Monday', 2 => 'Tuesday', 3 => 'Wednesday', 4 => 'Thursday', 5 => 'Friday', 6 => 'Saturday', 0 => 'Sunday'] as $key => $label)
                            <option value="{{ $key }}" {{ old('day_of_week') == (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <select id="shift_id" name="shift_id" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500 focus:ring-blue-500">
                        ...
                        @foreach($shifts as $shift)
                            <option value="{{ $shift->id }}" data-start="{{ $shift->start_time }}" data-end="{{ $shift->end_time }}" {{ old('shift_id') == $shift->id ? 'selected' : '' }}>{{ $shift->name }} ({{ \Carbon\Carbon::createFromFormat('H:i:s', $shift->start_time)->format('g:i A') }} - {{ \Carbon\Carbon::createFromFormat('H:i:s', $shift->end_time)->format('g:i A') }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <select id="doctor_id" name="doctor_id" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500 focus:ring-blue-500">
                        ...
                        @foreach($doctors as $doctor)
                            <option value="{{ $doctor->id }}" data-doctorschedule='@json($doctor->schedules->groupBy('day_of_week'))' {{ old('doctor_id') == $doctor->id ? 'selected' : '' }}>{{ $doctor->name }}@if($doctor->specialization) - {{ $doctor->specialization }}@endif</option>
                        @endforeach
                    </select>
                    <p class="text-xs text-slate-500">Only doctors scheduled for the selected day and shift appear here.</p>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <select id="staff_id" name="staff_id" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500 focus:ring-blue-500">
                        ...
                        @foreach($staff as $member)
                            <option value="{{ $member->id }}" {{ old('staff_id') == $member->id ? 'selected' : '' }}>{{ $member->name }}@if($member->designation) - {{ $member->designation }}@endif</option>
                        @endforeach
                    </select>
                    <p class="text-xs text-slate-500">Available staff are filtered by shift conflicts on the selected day.</p>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <select id="ward_id" name="ward_id" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500 focus:ring-blue-500">
                        ...
                        @foreach($wards as $ward)
                            <option value="{{ $ward->id }}" {{ old('ward_id') == $ward->id ? 'selected' : '' }}>{{ $ward->name }} @if($ward->code) ({{ $ward->code }})@endif</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <select id="room_id" name="room_id" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500 focus:ring-blue-500">
                        ...
                        @foreach($rooms as $room)
                            <option value="{{ $room->id }}" data-ward="{{ $room->ward_id }}" {{ old('room_id') == $room->id ? 'selected' : '' }}>{{ $room->room_no }}@if(optional($room->ward)->name) - {{ optional($room->ward)->name }}@endif</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <input id="start_time" type="time" name="start_time" value="{{ old('start_time') }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500 focus:ring-blue-500" required>
                </div>

                <div class="space-y-3">
                    <label class="block text-sm font-semibold text-slate-700">End Time</label>
                    <input id="end_time" type="time" name="end_time" value="{{ old('end_time') }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500 focus:ring-blue-500" required>
                </div>

                <div class="lg:col-span-2">
                    <label class="block text-sm font-semibold text-slate-700">Task / Responsibility</label>
                    <textarea name="task_description" rows="3" class="w-full rounded-3xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500 focus:ring-blue-500">{{ old('task_description') }}</textarea>
                </div>

                <div class="lg:col-span-2">
                    <label class="inline-flex items-center gap-3 rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4 w-full">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="h-5 w-5 text-blue-600 rounded focus:ring-blue-500 border-slate-300">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:justify-between">
                ...
                <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-blue-600 px-6 py-3 text-white font-semibold hover:bg-blue-700 transition">Save Roster Entry</button>
            </div>
        </form>
    </div>
</div>
