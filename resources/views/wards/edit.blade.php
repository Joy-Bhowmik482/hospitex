...
<div class="max-w-4xl mx-auto">
    <!-- Page Header -->
    <div class="mb-8">
        <h2 class="text-3xl font-bold text-slate-800 mb-2">Edit Ward</h2>
        <p class="text-slate-600">Update the ward information below.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-8">
        ...
        <form action="{{ route('wards.update', $ward) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="border-b border-slate-200 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="code" name="code" value="{{ old('code', $ward->code) }}" 
                    ...
                    <div>
                        ...
                        <input type="number" id="floor" name="floor" value="{{ old('floor', $ward->floor) }}" 
                    ...
                    <div>
                        ...
                        <select id="gender_policy" name="gender_policy" 
                            ...
                            required>
                            <option value="Male" {{ old('gender_policy', $ward->gender_policy) === 'Male' ? 'selected' : '' }}>Male Only</option>
                            <option value="Female" {{ old('gender_policy', $ward->gender_policy) === 'Female' ? 'selected' : '' }}>Female Only</option>
                            <option value="Any" {{ old('gender_policy', $ward->gender_policy) === 'Any' ? 'selected' : '' }}>Any Gender</option>
                        </select>
                        @error('gender_policy')
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                        @enderror
