...
<div class="max-w-7xl mx-auto">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h2 class="text-3xl font-bold text-slate-800 mb-2">Login History</h2>
            <p class="text-slate-600">Review recent login events and authentication activity for all users.</p>
        </div>
    ...
    @if ($logs->isEmpty())
        <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-12 text-center">
            ...
            <h3 class="text-xl font-semibold text-slate-800 mb-2">No Login Events</h3>
            <p class="text-slate-600">There are no login history records yet. Login attempts will appear here automatically.</p>
        </div>
    @else
                ...
                <table class="w-full text-left">
                    <thead class="bg-gradient-to-r from-blue-50 to-blue-100 border-b border-slate-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-xs font-semibold uppercase text-slate-700">IP</th>
                            <th class="px-6 py-4 text-xs font-semibold uppercase text-slate-700">User Agent</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-200">
                        @foreach ($logs as $log)
                            <tr class="hover:bg-slate-50 transition duration-150">
                                ...
                                <td class="px-6 py-4 text-slate-700">{{ $log->ip ?? 'N/A' }}</td>
                                <td class="px-6 py-4 text-slate-700">{{ \Illuminate\Support\Str::limit($log->user_agent ?? 'N/A', 60) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
