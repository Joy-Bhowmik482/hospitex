...
<div class="max-w-4xl mx-auto">
    <!-- Page Header -->
    <div class="mb-8">
        <h2 class="text-3xl font-bold text-slate-800 mb-2">Edit Bed Allocation</h2>
        <p class="text-slate-600">Update the bed allocation details below.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-8">
        ...
        <form action="{{ route('bed-allocations.update', $bedAllocation) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="border-b border-slate-200 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="admission_id" name="admission_id" 
                            ...
                            @foreach ($admissions as $admission)
                                <option value="{{ $admission->id }}" {{ old('admission_id', $bedAllocation->admission_id) == $admission->id ? 'selected' : '' }}>
                    ...
                    <!-- Bed -->
                    <div>
                        <label for="bed_id" class="block text-sm font-semibold text-slate-700 mb-2">Bed *</label>
                        <select id="bed_id" name="bed_id" 
                            ...
                            required>
                            @foreach ($beds as $bed)
                                <option value="{{ $bed->id }}" {{ old('bed_id', $bedAllocation->bed_id) == $bed->id ? 'selected' : '' }}>
                                    Bed {{ $bed->bed_no }} ({{ $bed->room->room_no }}, {{ $bed->room->ward->name }})
                                </option>
                            @endforeach
                        </select>
                        @error('bed_id')
            ...
            <div class="border-b border-slate-200 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" id="allocated_at" name="allocated_at" value="{{ old('allocated_at', $bedAllocation->allocated_at->format('Y-m-d\TH:i')) }}" 
                    ...
                    <div>
                        ...
                        <input type="datetime-local" id="released_at" name="released_at" value="{{ old('released_at', $bedAllocation->released_at?->format('Y-m-d\TH:i')) }}" 
                    ...
                    <div>
                        ...
                        <select id="allocation_status" name="allocation_status" 
                            ...
                            required>
                            <option value="Active" {{ old('allocation_status', $bedAllocation->allocation_status) === 'Active' ? 'selected' : '' }}>Active</option>
                            <option value="Released" {{ old('allocation_status', $bedAllocation->allocation_status) === 'Released' ? 'selected' : '' }}>Released</option>
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="notes" name="notes" rows="3" 
                            class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition">{{ old('notes', $bedAllocation->notes) }}</textarea>
                    </div>
                </div>
            </div>
