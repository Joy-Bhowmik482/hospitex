...
<div class="max-w-2xl mx-auto">
    <!-- Page Header -->
    <div class="mb-8">
        <h2 class="text-3xl font-bold text-slate-800">Edit Payment</h2>
        <p class="text-slate-500 mt-1">Update payment information</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('payments.update', $payment) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select id="invoice_id" name="invoice_id" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('invoice_id') border-red-500 @enderror">
                    ...
                    @foreach($invoices as $invoice)
                        <option value="{{ $invoice->id }}" {{ old('invoice_id', $payment->invoice_id) == $invoice->id ? 'selected' : '' }}>{{ $invoice->invoice_no }} - {{ number_format($invoice->net_total, 2) }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="date" id="paid_at" name="paid_at" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('paid_at') border-red-500 @enderror" value="{{ old('paid_at', $payment->paid_at->format('Y-m-d')) }}">
            ...
            <div class="mb-4">
                ...
                <input type="number" id="amount" name="amount" required step="0.01" min="0.01" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('amount') border-red-500 @enderror" value="{{ old('amount', $payment->amount) }}">
            ...
            <div class="mb-4">
                ...
                <select id="method" name="method" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <option value="bKash" {{ old('method', $payment->method) === 'bKash' ? 'selected' : '' }}>bKash</option>
                    <option value="Bank" {{ old('method', $payment->method) === 'Bank' ? 'selected' : '' }}>Bank Transfer</option>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="trx_id" name="trx_id" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('trx_id', $payment->trx_id) }}">
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="received_by" name="received_by" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('received_by') border-red-500 @enderror">
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ old('received_by', $payment->received_by) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <textarea id="notes" name="notes" rows="3" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('notes', $payment->notes) }}</textarea>
            </div>
