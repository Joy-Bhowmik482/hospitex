...
<div class="max-w-4xl mx-auto">
    <!-- Page Header -->
    <div class="mb-8">
        <h2 class="text-3xl font-bold text-slate-800 mb-2">Add New Role</h2>
        <p class="text-slate-600">Create a new role for user access control in the system.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-8">
        ...
        <form action="{{ route('roles.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}"
                            ...
                            placeholder="e.g., Admin, Doctor, Nurse" required>
                        @error('name')
                    ...
                    <!-- Slug -->
                    <div>
                        <label for="slug" class="block text-sm font-semibold text-slate-700 mb-2">Role Slug *</label>
                        ...
                        @error('slug')
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                        @enderror
                        <p class="text-xs text-slate-500 mt-1">Slug should be lowercase with hyphens (e.g., senior-doctor)</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between pt-6">
                <a href="{{ route('roles.index') }}" class="bg-slate-500 text-white font-semibold py-2 px-6 rounded-lg hover:bg-slate-600 transition">
                    Cancel
                </a>
