...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-3xl font-bold text-slate-900">Create Service</h1>
            <p class="text-slate-500 mt-1">Add a new charge item to the billing catalog.</p>
        </div>
        ...
        <form action="{{ route('services.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid gap-6 sm:grid-cols-2">
                <div>
                    ...
                    <input name="name" value="{{ old('name') }}" required class="mt-2 block w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Service Code</label>
                    <input name="code" value="{{ old('code') }}" required class="mt-2 block w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" />
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="4" class="mt-2 block w-full rounded-3xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
                ...
                <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-700">Save Service</button>
            </div>
        </form>
    </div>
</div>
